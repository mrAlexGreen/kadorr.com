(function( $ ){
  ...
  $.fn.toggles = function(opts) {
    ...
    return this.each(function() {
      ...
      var h = self.height()-4, w= self.width();
      if (h === 0 || w===0) {
        self.height(h = 20);
        self.width(w = 50);
      }
      var slide = $('<div class="slide"> <i class="icon icon_night" /> <i class="icon icon_day" /> </div>'), inner = $('<div class="inner" />'),on = $('<div class="on"> </div>'), off = $('<div class="off"> </div>'), blob = $('<div class="blob"> </div>');
      on
      .css({
        height:h,
        width: w-h/2,
      ...
      if (o.dragable) {
        blob.on('mousedown',function(e) {
          ...
          self.on('mousemove',blob,function(e) {
            ...
            if (slide.hasClass('active')) {
              inner.css({
                marginLeft: (diff < 0) ? (diff < -w+h+30) ? -w+h+30 : diff : 0
              });
            }else{
              inner.css({
                marginLeft: (diff > 0) ? (diff > w-h) ? 0 : diff-w+h+30 :-w+h+30
              });
            }
          });
